...
<main role="main" class="pb-3">
    ...
    <div class="d-print-block ">
       <h4 class="font-weight-light mb-5"><strong>{{$total[0]->ttl}}</strong> deduction request(s) Pending</h4>
       <table class="table table-striped table-sm table-hover border-bottom">
          ...
          <tbody>
            @foreach ($approved as $apprvd)
             <tr>
                <td><a href="{{route('corporate.pending_deduction_detail', ['record_ID' => $apprvd->record_ID])}}">{{$apprvd->record_ID}}</a></td>
                ...
                <td><i class='fa fa-paper-plane text-info'></i>
                    @if($apprvd->request_type == 0)
                    New
                    @elseif ($apprvd->request_type == 1)
                    Change
                    @else
                    Delete
                    @endif</td>
